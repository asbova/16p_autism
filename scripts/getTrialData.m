...
%             trial(j).rewardExitTimes = fzIn - curTE;
            ...
            switch_arrival = [];
            switch_depart = [];            
            if ~isempty(short_rel) && ~isempty(long_rsp) && any(long_rsp > min(short_rel)) && trialType(j) == 18000 && ~isempty(trial(j).reward)
...
%             end
        ...
        % adding in because of one weird trials
        bad_trials = [];
        for i_trial = 1 : size(trial,2)
            if trial(i_trial).SwitchDepart > 18
                bad_trials = i_trial;
            end
        end
        trial(bad_trials) = [];
        ...
        long_trials = find(cellfun(@(x) x == 18000, {trial.programmedDuration}));
        short_trials = find(cellfun(@(x) x == 6000, {trial.programmedDuration}));
        correct_trials = find(cellfun(@(x) ~isempty(x), {trial.reward}));
        
        trial(1).shortRatio = length(intersect(short_trials,correct_trials))/length(short_trials);
        trial(1).longRatio = length(intersect(long_trials,correct_trials))/length(long_trials);
